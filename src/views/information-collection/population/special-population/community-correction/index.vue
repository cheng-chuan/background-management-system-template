  ...
  <div class="app-container">
    ...
    <ZtViewLayout>
      ...
      <template #operate-left>
        ...
        <el-button size="mini" disabled>脱管漏管</el-button>
        <el-button size="mini" disabled>转刑满释放</el-button>
...
import { getList, delCommunityInfos } from '@/api/information-collection/population/special-population/community-correction'
...
export default {
  ...
  methods: {
    ...
    editPopulation(row, row2) {
      ...
      this.editConfig.communityData = row
    ...
    delOption(ids) {
      this.$confirm('确定删除选中信息?', '提示', {
        ...
        delCommunityInfos(ids).then(res => {
          console.log(res)
          if (res.status === 200 && res.data.code === '000000') {
            this.initTableList({ pageNo: 1, pageSize: 10 })
