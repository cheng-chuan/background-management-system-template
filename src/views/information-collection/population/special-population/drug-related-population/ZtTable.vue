...
<script>
import { getBasicDetail, delDrugInfo } from '@/api/information-collection/population/special-population/drug-related-population'
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      if (row.sffmstjd && row.sffmstjd === '1') {
        row.sffmstjd = true
      } else if (row.sffmstjd && row.sffmstjd === '0') {
        row.sffmstjd = false
      }
      if (row.sfywffzsPdbz && row.sfywffzsPdbz === '1') {
        row.sfywffzsPdbz = true
      } else if (row.sfywffzsPdbz && row.sfywffzsPdbz === '0') {
        row.sfywffzsPdbz = false
      }
    ...
    handleDelete(index, row) {
      this.$confirm('是否删除信息?', '提示', {
        ...
        delDrugInfo(row.xxzjbh).then(res => {
          console.log(res)
