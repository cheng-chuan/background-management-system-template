  ...
  >
    ...
    <zt-map ref="ztmap" v-bind="mapConf" @selectAddress="handleSelectAddress">
      <div style="height:500px;overflow:auto;padding:0 15px">
        ...
        <hr style="width:99%">
        <!-- 步骤一表单 -->
        <el-form ref="stepOneForm" :model="basicForm" label-width="110px">
          <el-row :gutter="24">
            <el-col :span="6">
            ...
            <el-col :span="6">
              <el-form-item label="有无固定住所:">
                <el-radio-group v-model="basicForm.ywgdzs">
          ...
          <el-row :gutter="24">
            <el-col :span="24">
              <el-form-item label="户籍地详址">
                <el-input v-model="basicForm.hjdxz" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            <el-col :span="6">
              <el-form-item label="未落户时间：">
                <el-date-picker
                  v-model="basicForm.wlhsj"
                  size="small"
                  type="datetime"
                  placeholder="选择日期时间"
                  style="width:100%"
            ...
            <el-col :span="6">
              <el-form-item label="未落户原因：">
                <ZtDicSelect v-model="basicForm.wlhyy" code="" width="100%" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="持证种类：">
                <ZtDicSelect v-model="basicForm.cyzjdm" code="" width="100%" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="证件编号：">
                <el-input v-model="basicForm.zjhm" size="small" placeholder="" />
              </el-form-item>
            </el-col>
          </el-row>
          <!-- <el-row :gutter="10">
...
<script>
import { addRegisteredPopulation, editRegisteredPopulation, searchAllDropDownBox, addRegisteredPopulationHK, deleteRegisteredPopulationZF, getHouseAddress } from '@/api/information-collection/population/real-population/registered-population'
  ...
  methods: {
    ...
    handleSelectAddress({ address, addressId, addressCode }) {
      if (this.mapConf.title === '选择现住地址') {
        this.$set(this.stepTwoForm, 'xzzDzbm', addressId)
        this.$set(this.stepTwoForm, 'xzzQhnxxdz', address)
        this.$set(this.stepTwoForm, 'xzzXzqhdm', addressCode)
      } else if (this.mapConf.title === '选择户籍地址') {
        this.$set(this.stepTwoForm, 'hjdzDzbm', addressId)
        this.$set(this.stepTwoForm, 'hjdzQhnxxdz', address)
        this.$set(this.stepTwoForm, 'hjdzXzqhdm', addressCode)
      }
    },
    ...
    // 获取地址下拉框
    getHouseAddress(typeStr, name) {
      const params = { name, typeStr }
      getHouseAddress(params).then(res => {
        console.log(res)
        if (typeStr === '1') {
          this.allList.provinceList = res.data.data
        } else if (typeStr === '2') {
          this.allList.cityList = res.data.data
        } else if (typeStr === '3') {
          this.allList.countyList = res.data.data
        }
      })
    },
    jlxmcChange(val) {
